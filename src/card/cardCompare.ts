...
function CardComparer() {
        ...
        if (set1D[0].priority > set2D[0].priority) {
            ...
            if (set1D[1].priority > set2D[1].priority) {
                ...
                if (set1D[2].priority > set2D[2].priority) {
                    ...
                    return (
                        _options.wininingPriority.cardType[set1D[2].type]
                            .priority >
                        _options.wininingPriority.cardType[set2D[2].type]
                            .priority
                    );
                }
            }
    ...
    function isHighPair(set1, set2) {
        ...
        if (pair1Rank > pair2Rank) {
            ...
            if (last1Card.priority > last2Card.priority) {
                ...
                return (
                    _options.wininingPriority.cardType[last1Card.type]
                        .priority >
    ...
    function getGreatestFromType(type, sets, set_prop) {
        let setProp = set_prop || 'set';
        switch (type) {
            case 'trail':
    ...
    this.getGreatest = function (sets, set_prop) {
        let arrNew = [],
            sorted,
            setProp = set_prop || 'set',
            maxP = -1;
        for (let count = 0, len = sets.length; count < len; count++) {
            let setType = this.getSetType(sets[count][setProp]);
